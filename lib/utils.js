...
export const UNSUPPORTED_ENVIRONMENTS = [
    {
        platform: 'darwin',
        architecture: 'arm64',
    },
 ...
 */
export const getOptionalString = (name, defaultValue = '') => getInput(name, { required: false }) || defaultValue;
/**
 ...
 */
export function downloadToFile(url, file, mode = 0o755) {
 ...
 */
export async function getFileContents(filePath, options) {
 ...
 */
export async function getFileContentsAsString(filePath, options) {
 ...
 */
export async function getFileChecksum(filePath, algorithm = 'sha256') {
 ...
 */
export async function verifyChecksum(originalFile, checksumFile, algorithm = 'sha256') {
    ...
    try {
        return timingSafeEqual(Buffer.from(binaryChecksum), Buffer.from(declaredChecksum));
    }
    catch {
 ...
 */
export async function verifySignature(messageFilePath, signatureFilePath, publicKeyFilePath) {
 ...
 * @returns
 */
export function parsePathAndFormat(coverageConfigLine) {
...
export function getSupportedEnvironmentInfo() {
    ...
    return {
        supported: !UNSUPPORTED_ENVIRONMENTS.some((e) => {
            return (e.architecture === currentEnvironment.architecture &&
