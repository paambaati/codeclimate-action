...
test('🧪 downloadToFile() should download the give URL and write to given file location with given mode.', async t => {
    ...
    await downloadToFile('http://localhost.test/dummy-cc-reporter', filePath, 0o777);
    stat(filePath, (err, stats) => {
        if (err) t.fail(err.message);
        t.equal(stats.mode, 33261, 'downloaded file should exist and have executable permissions.');
        unlinkSync(filePath);
        nock.cleanAll();
        t.end();
    });
...
test('🧪 run() should run the CC reporter (happy path).', async t => {
    ...
    try {
        await run('http://localhost.test/dummy-cc-reporter', filePath, `echo 'coverage ok'`);
        unhookIntercept();
    } catch (err) {
        unhookIntercept();
        nock.cleanAll();
